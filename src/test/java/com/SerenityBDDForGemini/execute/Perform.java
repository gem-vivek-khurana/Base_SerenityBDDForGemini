...
public class Perform extends PageObject {
    ...
    public void settingFieldValue(By byField, String value) {
        settingFieldValue(getDriver().findElement(byField), value);
    }
    ...
    public void settingAngularTypeaheadAs(Field typeaheadField, Class<?> pageClass, String value) {
        By fieldToSetTypeahead = fieldToInteract(typeaheadField, pageClass);
        waitForCondition().until(ExpectedConditions.elementToBeClickable(fieldToSetTypeahead));
        settingAngularTypeaheadAs(fieldToSetTypeahead, value);
    }

    public void settingAngularTypeaheadAs(By typeaheadBy, String value) {
        WebElement typeaheadElement = getDriver().findElement(typeaheadBy);
        settingAngularTypeaheadAs(typeaheadElement, value);
    }

    public void settingAngularTypeaheadAs(WebElement typeaheadElement, String value) {
        clickOn(typeaheadElement);
        WebElement textField = typeaheadElement.findElement(By.className("mat-input-element"));
        settingFieldValue(textField, value);
        try {
            Thread.sleep(1500);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }
        List<WebElement> matOptions = typeaheadElement.findElements(By.cssSelector("mat-option"));
        for (WebElement matOption : matOptions) {
            if (matOption.getAttribute("ng-reflect-value").equalsIgnoreCase(value)) {
                matOption.click();
            }
        }
    }
